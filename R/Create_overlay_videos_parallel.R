...
create_overlays_parallel <- function(process_ID, video.files.df, traj.data, to.data, merged.data.folder, raw.video.folder, temp.overlay.folder, overlay.folder, 
  ...
  for (i in 1:length(file_names)) {
    dir.create(paste0(to.data, temp.overlay.folder, process_ID, "/", file_names[i]), showWarnings = F)
    traj.data_tmp <- subset(traj.data, file == file_names[i])
    j <- 1
    
    if (type == "traj") {
      while (j <= max(traj.data$frame)) {
        jpeg(paste(to.data, temp.overlay.folder, process_ID, "/", file_names[i], "/", "frame_", j, ".jpg", sep = ""), width = as.numeric(width), height = as.numeric(height), quality = 100)
    ...
    if (type == "label") {
      while (j <= max(traj.data$frame)) {
        jpeg(paste(to.data, temp.overlay.folder, process_ID, "/", file_names[i], "/", "frame_", 
  ...
  text[grep("overlay_input = ", text)] <- paste("overlay_input = ", "'", paste0(to.data, temp.overlay.folder, process_ID, "/"), "';", sep = "")
